...
class L2Adversary(object):
    ...
    def __init__(self, targeted=True, confidence=0.0, c_range=(1e-3, 1e10),
                 ...
                 box=(-1., 1.), optimizer_lr=1e-2, init_rand=False,device='cuda'):
        """
    ...
    def __call__(self, model, inputs, targets, to_numpy=True):
        ...
        targets_oh = torch.zeros(targets.size() + (num_classes,),device=self.device)  # type: torch.FloatTensor
        ...
        pert_tanh = torch.zeros(inputs.size(),device=self.device)  # type: torch.FloatTensor
        if self.init_rand:
        ...
        for sstep in range(self.binary_search_steps):
            ...
            scale_consts = torch.from_numpy(np.copy(scale_consts_np)).float().to(self.device)  # type: torch.FloatTensor
